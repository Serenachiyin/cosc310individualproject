...
public class Chatbot {
	...
	public void processInput(String userInput){	
		try { //translator API called
			userInput = Translator.translate("", "en", userInput);
			gui.sendMessage(userInput);
		...
		if (userInput.toLowerCase().contains("location")||userInput.toLowerCase().contains("located")) {
			try{
				//place API url here
				String path = "https://maps.googleapis.com/maps/api/staticmap?center=ubco&zoom=13&scale=1&size=600x300&maptype=roadmap&key=AIzaSyBGJWY77lDXjrFyDFu63JugaueKFuUTY2k&format=png&visual_refresh=true";
			...
			if (userInput.toLowerCase().contains("country")) {
				try{
					//place API url here
					gui.sendMessage("Our country is Canada.");		
					String path = "https://maps.googleapis.com/maps/api/staticmap?center=canada&zoom=4&scale=1&size=600x300&maptype=roadmap&key=AIzaSyBGJWY77lDXjrFyDFu63JugaueKFuUTY2k&format=png&visual_refresh=true";
					URL url = new URL(path);
					BufferedImage image = ImageIO.read(url);
					JLabel label = new JLabel(new ImageIcon(image));
					JFrame f = new JFrame();
					...
					f.setLocation(200,200);
					f.setVisible(true);
					System.out.println("Country works!");
				}
				catch (Exception ex) {
					System.out.println("Country Error!" + ex);
				}	
				}

				if (userInput.toLowerCase().contains("city")) {
					try{
						//place API url here
						gui.sendMessage("We are located in the City of Kelowna.");		
						String path = "https://maps.googleapis.com/maps/api/staticmap?center=kelowna&zoom=13&scale=1&size=600x300&maptype=roadmap&key=AIzaSyBGJWY77lDXjrFyDFu63JugaueKFuUTY2k&format=png&visual_refresh=true";
						URL url = new URL(path);
						BufferedImage image = ImageIO.read(url);
						JLabel label = new JLabel(new ImageIcon(image));
						JFrame f = new JFrame();
						...
						System.out.println("City Error!" + ex);
					}	
					}
		...
		if(current_node.hasFlow() && !flowComplete) processFlow(userInput);
			...
			if (potentialPath != Root) { // If previous input resulted in potentialPath, check if user says yes or no
				...
				if (starting_node == current_node) {
					...
					if (userInput.toLowerCase().contains("location")||userInput.toLowerCase().contains("located")) {
						gui.sendMessage("Here is the location of our company: 3333 University Way, Kelowna, BC V1V 1V7");		
					}
	...
	void initializeResponses() {
		System.out.println(getClass().getClassLoader().getResource("Responses.txt").getFile());
		int lineError = 0;	
		String whole = null;	
